...
class DailyCallManager {
  ...
  async createCombinedVideoStream() {
    // Enumerate all video devices
    const devices = await navigator.mediaDevices.enumerateDevices();
    const videoDevices = devices.filter((device) => device.kind === "videoinput");
  
    // Check the number of available video devices
    if (videoDevices.length === 0) {
      console.error("No video devices found.");
      return;
    } else if (videoDevices.length === 1) {
      console.warn("Only one video device found. Using single device without combination.");
      // If only one device is available, use it directly as the video source
      const singleStream = await navigator.mediaDevices.getUserMedia({ video: { deviceId: videoDevices[0].deviceId } });
      this.combinedStream = singleStream; // Use single device as the stream
      return;
    }
  
    // If more than one video device is available, use the first two devices for combining
    const [device1, device2] = videoDevices;
  
    // Access the video inputs based on device IDs
    const [stream1, stream2] = await Promise.all([
      navigator.mediaDevices.getUserMedia({ video: { deviceId: device1.deviceId } }),
      navigator.mediaDevices.getUserMedia({ video: { deviceId: device2.deviceId } })
    ]);
  
    // Create <video> elements for each camera feed
    ...
    // Create a canvas and context for compositing the video feeds
    const canvas = document.createElement("canvas");
    const ctx = canvas.getContext("2d");
  
    // Set canvas size to match the desired resolution
    canvas.width = 1280; // Adjust as necessary
    canvas.height = 720;
  
    // Function to draw the video feeds onto the canvas
    const drawFrame = () => {
      ctx.clearRect(0, 0, canvas.width, canvas.height);
  
      // Draw each video feed to a different side of the canvas
    ...
    // Capture the canvas stream and use it as the combined video stream
    const combinedVideoTrack = canvas.captureStream().getVideoTracks()[0];
    this.combinedStream = new MediaStream([combinedVideoTrack]);
  }
  ...
  async joinRoom(roomUrl, joinToken = null) {
    ...
    try {
      ...
      // Create the combined video stream before joining the room
      await this.createCombinedVideoStream();

      // Replace the local video track with the combined video stream
      this.call.setInputDevicesAsync({ videoSource: this.combinedStream.getVideoTracks()[0] });
